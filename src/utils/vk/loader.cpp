...
loadImage(Engine *engine, fastgltf::Asset &asset, const fastgltf::Image &image,
  ...
  std::visit(
      fastgltf::visitor{
          /// @note const specifier should be consistent with variable.
          [](const auto &) { fmt::println("here"); },
          [&](const fastgltf::sources::URI &filePath) {
            ...
            path = parent_path.string() + "/" + path;
            fmt::println("file path {}", path.c_str());
            unsigned char *data =
...
loadGltf(Engine *engine, std::filesystem::path file_path) {
  ...
  for (const fastgltf::Image &image : gltf.images) {
    std::optional<AllocatedImage> img =
        loadImage(engine, gltf, image, path.parent_path());
    if (img.has_value()) {
  ...
  // A big buffer for all material constants.
  scene->material_constants_buffer = engine->createBuffer(
  ...
  GLTFMetallicRoughness::MaterialConstants *scene_material_constants =
      (GLTFMetallicRoughness::MaterialConstants *)
          scene->material_constants_buffer.alloc_info.pMappedData;
  ...
  for (const fastgltf::Material &mat : gltf.materials) {
    ...
    material_res.data_buffer = scene->material_constants_buffer.buffer;
